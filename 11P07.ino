...
void loop() {
  ...
  // add your code here!
  if(dist_raw<=_TARGET_LOW) pinMode(PIN_LED, INPUT);
  else if(dist_raw<_TARGET_HIGH) pinMode(PIN_LED, OUTPUT);
  else pinMode(PIN_LED, INPUT);
  myservo.writeMicroseconds((dist_ema-_TARGET_LOW)/(_TARGET_HIGH-_TARGET_LOW)*(_DUTY_MAX-_DUTY_MIN)+_DUTY_MIN);
